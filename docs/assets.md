...
| `ui/monitor/footman.png` | 步兵头像（WC3 图标，MonitorPage 步兵渲染用） |
| `ui/monitor/footman_walk.png` | 行走循环精灵表（AI 生成绿幕图经 `tools/sprite_extract.py` 抠图重排，4 行 × 8 列、单元格 204×176，行序 下/上/左/右）：小尺寸下不够清晰，**当前未使用**，留作备用 |
