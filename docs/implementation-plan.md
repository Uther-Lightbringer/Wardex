...
1. **war 控件库**（`src/components/war/`）：WarFrame（border-image 九宫格 + hole/inset 开孔）、WarButton（三态+音效+uiGate）、WarDialog（标题板/按钮区贴图分数定位）、WarDropdown、WarMenu、WarScrollBar（置黑禁用）、吊链平铺、蓝色高亮 mix-blend。每个控件的 border 值严格从 ui-design.md 抄。
