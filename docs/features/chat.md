...
- **segments 交错渲染**（`ui/ChatBubble.qml:379-517`）：
  ...
  - `tool` 段：单行 `▶/▼ · <名称>  [<status>]`（底 `#4412151c`、边 `#3a4a40`），**默认折叠**，点击展开 pretty-print 的 payload。名称取 `name || title || kind || "tool"`（`ui/ChatBubble.qml:192-195`）。**新版变更：参数与结果分两块展示**——「调用参数」取 `rawInput || arguments || content || output`，「执行结果」取 `rawOutput || result`，均为空则不渲染对应块，折叠时标题行用 `参数 · 结果 ▾` 提示有内容可看；对象则 `JSON.stringify(,,2)`。**展开内容超过 64KB 时截断并追加 `…（已截断）`；完整 payload 只在磁盘 JSONL 中保留**（旧版不截断，长 payload 会撑爆 UI）。
  - **pi 侧的参数补发**：pi 的 `tool_execution_*` 事件只在 start/update 带 `args`，结束事件不带；`chat/pi.rs::on_tool_exec` 按 toolCallId 记住参数，三次 emit 都补 `rawInput`（pretty JSON 文本），否则已完成的行只有工具名、没有参数。
...
- **选中**（点击 / Enter / Tab）：确认后该 `@` 引用从正文移除，转为引用条（RefBar）里的一个 chip（见下「发送时展开」）；↑↓ 移动高亮，Esc 关闭。底部提示固定文案：`↑↓ 选择 · Enter 确认 · Esc 关闭 · 确认后成为上方引用条，可补 :起-止 行号`。
- **行号语法**：`@path:10`（仅第 10 行）、`@path:10-50`（区间）。
- **发送时展开**（`Composer.vue` 的 `expandRefs`/`expandRef`）：只有被显式确认的引用条 chip（选择器选中 / 精确命中 / 预览右键 `insertRef` / 拖入文件）会展开；正文里的裸 `@`（git@…、邮箱、@账号）一律原样发送、绝不当引用。展开时**不注入文件内容**：每个 chip 输出一行标记 + 一句指令，让模型用其自身的 read 工具读取（agent 以 cwd=项目根运行，相对路径可直接解析；推理时读取拿到的才是最新磁盘内容，而非发送瞬间的快照）：
...
- **读取/校验规则**（`store/workspace.rs` 的 `read_file_range`，发送前由前端 `Composer.vue` 调用）：发送时仅用它**校验**引用的路径/行号（存在性、工作区包含性、行范围），校验通过则丢弃内容、只发标记；校验失败则替换为该引用的失败占位（见下），不阻断发送。路径必须在工作区内（清理后绝对路径前缀校验，Windows 忽略大小写；绝对路径直接拒绝）→ 否则 `escape`；文件不存在 `missing`；不可读 `unreadable`；已知二进制/图片扩展或内容含 NUL → `binary`；`from<=0` 读全文；`to<=0` 读 `from` 单行；`from>总行数` → `range`；`to` 超界钳到末行。UTF-8 解码失败（含替换字符）回退系统本地编码（GBK）。
- **失败占位**：校验失败时该引用展开为单行说明，不阻断发送：`【引用文件：path：文件不存在或不可读】` / `二进制文件，已跳过` / `路径超出工作区，已拒绝` / `行范围超出文件（共 N 行）`。
...
- **提交历史**（只读）：`git log` 拉取最近 200 条，列表撑满面板高度，超出内部滚动。每行：`subject` + `shortHash · author · date`。`刷新` 链接（加载中显示 `加载中…`）；错误红字；空显示 `暂无提交`。刷新时机同分支徽标（手动点刷新亦可）。新版用 Rust 侧执行 `git log --format` 或读 git 库，交互语义不变。
...
旧 `reminders.json` 会话提醒已并入统一待办模型（`store/todos.rs`，格式见 data-formats.md §8.1）。右栏「待办」面板 = 上下文视图，随当前会话走：

- **三组**：本会话（scope=session，本会话的会话级行）/ 本项目（scope=project，当前会话所在项目）/ 全局（scope=global）。每组小标题 + 计数；`＋ 添加待办` 弹 TodoDialog（内容 + 范围选择 + 可选到期分钟数；无会话/项目上下文时对应范围禁用）。
- **到期行为**：`popup` 行（会话级 + 全局，manager 级 30s 到期扫描）→ 系统通知（窗口失焦时）+ 应用内弹窗（DueTodoOverlay，`todos://due` 事件），行保持 pending 直到用户勾掉。`project` 行到期 → 后端自动在项目内新建会话（命名=待办标题，不抢占当前活跃会话），行 settle 为 done，发 `todos://projectDue` 三选弹窗：**跳转并处理**（切到新会话 + 以 kind=reminder 发送待办内容触发推理）/ **后台处理**（不切换，直接后台发送）/ **我知道了**（仅保留会话），弹窗内可改会话命名（默认=待办标题）。
- **MCP agent 自我唤醒**：`set_reminder` 写入的仍是 scope=session、notifyMode=`push` 的行——到期由所属会话 runtime 作为 prompt 发回会话（kind=reminder），语义与旧版一致，现有提示词用法不受影响。
...
- [ ] 引用条展开为 `【引用文件：…】` 标记 + read 指令（模型自读最新内容）；escape/missing/unreadable/binary/range 五种失败占位；GBK 回退
