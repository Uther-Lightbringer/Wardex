...
参照 `ui/TodoPage.qml`（314 行）、`src/TodoStore.h/.cpp`。

- 应用级个人待办看板，持久化 `todos.json`（见 [../data-formats.md](../data-formats.md) §8）。
- 两区：**待办** / **已完成**；完成项划线 + 灰化，移入已完成区。
- 操作：新增（输入框回车）、勾选完成/取消完成、删除单条、**清除已完成**（一键清空已完成区）。
- 与聊天/会话数据完全独立，不含任何 ACP 交互。
