...
- **绿色便携版**：`build-portable.bat`（前置：已构建 `src-tauri/target/release/wardex-tauri.exe`）。把 exe + `background.example.json` 拷进 `dist-portable/`，并把 pi dist 镜像同步到 `dist-portable/resources/pi/packages/coding-agent/dist`（`locate_plugin_dir` 会查 `<exe目录>/resources/pi`，无需改代码），再拷 `pi-extensions` 到 `dist-portable/resources/pi-extensions`，最后打 `WarDex-win64-<版本>.zip`。不跑 bump-version，不动 NSIS 安装器。**该 bat 必须保持纯 ASCII + CRLF**（cmd 按 OEM 代码页解析 .bat，UTF-8 中文注释会吞字符导致解析错乱，注释用英文写）。
